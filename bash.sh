...
curl -o proxies_config.json https://raw.githubusercontent.com/gruzilkinnn/dev_bash/main/proxies_config.json 
#wget -N https:/raw.githubusercontent.com/gruzilkinnn/dev_bash/main/proxies_config.json
...
do
   ...
   list_size=$(curl -s https:/raw.githubusercontent.com/gruzilkinnn/dev_bash/main/runner_targets | cat | grep "^[^#]" | wc -l)
   ...
   for (( i=1; i<=list_size; i++ ))
      do
            cmd_line=$(awk 'NR=='"$i" <<< "$(curl -s https:/raw.githubusercontent.com/gruzilkinnn/dev_bash/main/runner_targets  | cat | grep "^[^#]")")
