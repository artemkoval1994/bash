...
do
   ...
   list_size=$(curl -s https:/raw.githubusercontent.com/dep-x/bash/main/runner_targets | cat | grep "^[^#]" | wc -l)
   ...
   for (( i=1; i<=list_size; i++ ))
      do
            cmd_line=$(awk 'NR=='"$i" <<< "$(curl -s https:/raw.githubusercontent.com/dep-x/bash/main/runner_targets  | cat | grep "^[^#]")")
